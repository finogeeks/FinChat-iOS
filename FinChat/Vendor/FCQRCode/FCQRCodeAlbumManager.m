...
#import "FCQRCodeAlbumManager.h"
#import <AVFoundation/AVFoundation.h>
#import <Photos/Photos.h>
#import "UIImage+ImageSize.h"

#ifdef DEBUG
#define FCQRCodeLog(...) NSLog(__VA_ARGS__)
#else
#define FCQRCodeLog(...)
#endif

@interface FCQRCodeAlbumManager () <UINavigationControllerDelegate, UIImagePickerControllerDelegate>
@property (nonatomic, strong) UIViewController *currentVC;
@property (nonatomic, strong) NSString *detectorString;
@end

@implementation FCQRCodeAlbumManager

+ (instancetype)sharedManager {
    static FCQRCodeAlbumManager *manager = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        manager = [[FCQRCodeAlbumManager alloc] init];
    });
    return manager;
}
...
- (void)readQRCodeFromAlbumWithCurrentController:(UIViewController *)currentController {
    ...
    if (currentController == nil) {
        NSException *excp = [NSException exceptionWithName:@"FCQRCode" reason:@"readQRCodeFromAlbumWithCurrentController: 方法中的 currentController 参数不能为空" userInfo:nil];
        [excp raise];
    }
...
- (void)imagePickerController:(UIImagePickerController *)picker didFCishPickingMediaWithInfo:(NSDictionary<NSString *,id> *)info {
    ...
    if (features.count == 0) {
        ...
        [self.currentVC dismissViewControllerAnimated:YES completion:^{
            if (self.delegate && [self.delegate respondsToSelector:@selector(QRCodeAlbumManager:didFCishPickingMediaWithResult:)]) {
                [self.delegate QRCodeAlbumManager:self didFCishPickingMediaWithResult:self.detectorString];
            }
        }];
    }
}
