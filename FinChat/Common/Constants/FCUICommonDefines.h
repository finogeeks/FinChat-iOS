//
//  FINUICommonDefines.h
//  FinoCommon
//
//  Created by 杨涛 on 2017/12/28.
//  Copyright © 2017年 finogeeks.com. All rights reserved.
//

#ifndef FCUICommonDefines_h
#define FCUICommonDefines_h
...
// 设备类型
#define IS_IPAD [FCUIHelper isIPad]
#define IS_IPAD_PRO [FCUIHelper isIPadPro]
#define IS_IPOD [FCUIHelper isIPod]
#define IS_IPHONE [FCUIHelper isIPhone]
#define IS_SIMULATOR [FCUIHelper isSimulator]
...
// 屏幕宽度，会根据横竖屏的变化而变化
#define FIN_SCREEN_WIDTH ([[UIScreen mainScreen] bounds].size.width)

// 屏幕宽度，跟横竖屏无关
#define FIN_DEVICE_WIDTH (IS_LANDSCAPE ? [[UIScreen mainScreen] bounds].size.height : [[UIScreen mainScreen] bounds].size.width)

// 屏幕高度，会根据横竖屏的变化而变化
#define FIN_SCREEN_HEIGHT ([[UIScreen mainScreen] bounds].size.height)

// 屏幕高度，跟横竖屏无关
#define FIN_DEVICE_HEIGHT (IS_LANDSCAPE ? [[UIScreen mainScreen] bounds].size.width : [[UIScreen mainScreen] bounds].size.height)

// 设备屏幕尺寸
#define IS_58INCH_SCREEN [FCUIHelper is58InchScreen]
#define IS_55INCH_SCREEN [FCUIHelper is55InchScreen]
#define IS_47INCH_SCREEN [FCUIHelper is47InchScreen]
#define IS_40INCH_SCREEN [FCUIHelper is40InchScreen]
#define IS_35INCH_SCREEN [FCUIHelper is35InchScreen]
...
// 获取一个像素
#define PixelOne [FCUIHelper pixelOne]
...
#define Fin_IsiPhoneX ([UIScreen instancesRespondToSelector:@selector(currentMode)] ? CGSizeEqualToSize(CGSizeMake(1125, 2436), [[UIScreen mainScreen] currentMode].size) : NO)


//适配 iOS11 ScrollView顶到安全区域下面的时候的bug
#define Fino_AdjustsScrollViewInsets_NO(scrollView,vc)\
do { \
...
_Pragma("clang diagnostic push") \
_Pragma("clang diagnostic ignored \"-Warc-performSelector-leaks\"") \
if ([keyWindow respondsToSelector:NSSelectorFromString(@"safeAreaInsets")] && Fin_IsiPhoneX) {\
...
#define UIColorMakeWithHEXString(x) [UIColor fin_colorWithHexString:x]
...
#define FINUIViewAnimationOptionsCurveOut (7<<16)
#define FINUIViewAnimationOptionsCurveIn (8<<16)
 ...
 *  issue: https://github.com/FINUI/FINUI_iOS/issues/203
 */
CG_INLINE CGFloat
